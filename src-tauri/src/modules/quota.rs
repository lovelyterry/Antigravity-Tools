    ...
    for (ep_idx, ep_url) in QUOTA_API_ENDPOINTS.iter().enumerate() {
            ...
            .await
        {
            Ok(response) => {
                ...
                // Register dynamically discovered models into the global common model registry (batched)
                crate::modules::common_models::register_models_batch(
                    quota_response.models.iter().map(|(name, info)| {
                        (
                            name.as_str(),
